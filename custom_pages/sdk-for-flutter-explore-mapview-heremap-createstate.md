...
The framework can call this method multiple times over the lifetime of a `StatefulWidget`. For example, if the widget is inserted into the tree in multiple locations, the framework will create a separate `State` object for each location. Similarly, if the widget is removed from the tree and later inserted into the tree again, the framework will call <a href="sdk-for-flutter-explore-mapview-heremap-createstate">createState</a> again to create a fresh `State` object, simplifying the lifecycle of `State` objects.
