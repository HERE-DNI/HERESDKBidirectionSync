...
- <div>
  ...
  Index of the <a href="sdk-for-ios-navigate-classes-span">`Span`</a> in the route section.
...
- <div>
  ...
  The progress for each <a href="sdk-for-ios-navigate-classes-section">`Section`</a> from the current one to the last one. Note that the progress information is accumulated successively, therefore information relative to the final destination is in the last item of the list. The list is guaranteed to be non-empty.
...
- <div>
  ...
  The progress for next and next-next maneuvers (see <a href="sdk-for-ios-navigate-classes-maneuver">`Maneuver`</a>). Note that the list can contain at maximum two items (for next and next-next maneuvers) and one or zero when approaching the destination.
